package io.higgs.boson.serialization.v2;
...
public class BosonWriteReadTest {
    ...
    public void readWriteWithJSON() {
        ...
        ByteBuf objWritten = writer.serialize(obj);
        ByteBuf arrPrimWritten = writer.serialize(arrPrim);
        ByteBuf arrObjsWritten = writer.serialize(arrObjs);
        ByteBuf arrMixedWritten = writer.serialize(arrMixed);

        ObjectNode objRead = reader.deSerialize(objWritten);
        ArrayNode arrPrimRead = reader.deSerialize(arrPrimWritten);
        ArrayNode arrObjRead = reader.deSerialize(arrObjsWritten);
        ArrayNode arrMixedRead = reader.deSerialize(arrMixedWritten);
